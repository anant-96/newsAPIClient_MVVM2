...
import com.example.newsapiclient_mvvmproj2.databinding.FragmentNewsBinding
        ...
        viewModel.newsHeadlines.observe(viewLifecycleOwner) { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let {
                        Log.d("MUTAG", "came here ${it.articles.toList().size}")
    ...
    private fun setSearchView() {
        fragmentNewsBinding.svNews.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query1: String?): Boolean {
                viewModel.searchNews("us", query1.toString(), page)
                viewSearchedNews()
                return false
            }
            ...
            override fun onQueryTextChange(query2: String?): Boolean {
                MainScope().launch {
                    delay(2000)
                    viewModel.searchNews("us", query2.toString(), page)
                    viewSearchedNews()
                }
                return false
            }
